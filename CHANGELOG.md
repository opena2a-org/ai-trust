...
- **Nothing looked at the bytes. A blocking `review` job now sits between `build` and `publish`, and it refuses to go green when it cannot see.** `consumer-audit` asks what the *manifest* resolves to; nothing on the release path asked what was actually *in* the tarball — which files, whether it runs on a clean machine, whether a key rode along in `dist/`. Those are properties of the artifact, so the only place to measure them is after `npm pack` and before `npm publish`, and after publish there is no undo (npm refuses to unpublish a version with dependents past 72 hours, and the bytes are on every mirror by then). The chain is now `consumer-audit -> build -> review -> publish -> github-release`; `review` holds `contents: read`, re-verifies the artifact's sha256 against `build`'s output before reading it, and runs `scripts/release-artifact-review.mjs --tarball <path>`. `publish` still holds `id-token: write` alone and still checks out nothing — `review` re-exports build's outputs so `publish` can `needs: review` without silently reading empty strings for the tarball name and digest.
  The script reports one of three outcomes per check — `pass`, `fail`, or **`precondition: <what is missing>`** — and prints a census line naming *every* check whatever it said, so "was that check even run" is answerable from the log rather than from the source. **A precondition exits non-zero, exactly like a failure.** That is the whole design: the failure mode being defended against is the one `audit-consumer-resolution.mjs` documents at length, where a gate that cannot take its measurement (no network, no scanner, no advisory database) reports zero findings and reads as green. Nine checks: entries confined to `dist/` plus the three files npm adds; no dotfiles; no `__tests__`/`fixtures`/`test/`; no `preinstall`/`install`/`postinstall` in the packed manifest; no caret or tilde on an `@opena2a/` dependency; a clean-global-prefix smoke run of `--version` and `--help` with an empty `HOME` and the network taken away inside the child; `npm audit --omit=dev` at high-or-above against the same shared acceptance list the other two gates read; a `hackmyagent` credential scan of a FLAT scratch copy of `dist/` (no `dist` path component — the scanner's own walk skips directories named `dist`, so a copy that kept the prefix would be a scan of nothing that reads as clean) **with a planted control file**, so "zero findings" is only accepted as an answer when the control was found; and the ruling's consumer-closure check, which resolves the fresh-install closure of the tarball and, for every own-package copy in it (the roster plus the `@opena2a/` scope, the packed package itself included), reads its registry deprecation and its repository's GitHub security advisories live — with two instrument self-tests (a named known-deprecated version must read as deprecated; the advisory feed must be non-empty across the roster's repositories) so "no findings" can never mean "did not look". The check takes `--advisory-states published|all` (the workflow passes `published`; draft and triage advisories of other repositories are not readable by its token), and the step that runs it carries `GH_TOKEN` so a shared unauthenticated rate budget cannot surface as a blocking precondition. On ai-trust's own pins this check is EXPECTED to fail — the pinned `hackmyagent@0.25.0` is deprecated and inside published advisory ranges, as are its nested own copies — and that failure blocks the release job, which is the check doing its job.
  The planted credential control is a single credential-named `const` whose value is assembled at runtime from parts (`sk-` + `proj-` + 48 repeated `A`s), so no credential-shaped literal exists in this repository while the scratch file carries the assembled form the scanner's canonical credential walk actually keys on (AST-CRED-001/AST-CRED-003, measured on the pinned hackmyagent 0.25.0). The AWS documentation example pair planted previously was inadmissible — 0.25.0 does not flag it, so it proved nothing about the instrument. And because the scanner collapses duplicate checkIds onto the first file in walk order, a credential in a shipped file shadows the control: the check judges shipped findings FIRST, so a caught credential reads as the `fail` it is, not as a "control not flagged" precondition blaming the instrument for working.
  The tarball is parsed in-process rather than by shelling out to `tar`: the input is the thing under review, so it does not get to influence a command line, and the result does not depend on which `tar` the runner ships. Every class the script claims to catch was made to fail first, one poisoned tarball each, and the assertion is that the census names *that* check and no other — a script that failed everything would satisfy a weaker test and catch nothing.
- **Three `@opena2a/` dependencies were on caret ranges, so a sibling repo's release could change what a published `ai-trust` resolves to.** `@opena2a/ai-classifier`, `@opena2a/contribute` and `@opena2a/shared` were declared `^0.1.0` while `cli-ui`, `check-core`, `registry-client` and `telemetry` were already exact. A caret on a first-party package means someone else's publish moves our users' tree with no tag, no review and no test run here. All three are now pinned to the version the committed lockfile already resolved (`0.1.1`, `0.1.1`, `0.1.2`): **the installed tree is byte-identical**, this is a narrowing of the declared range rather than an upgrade, and the lockfile's root `dependencies` map was updated in step so `npm ci` still validates. The new `pinned-first-party-deps` check is what keeps it that way.
- **Every job that runs `npm ci` now refuses to install with a tracked package-manager config in the tree.** A committed `.npmrc`, `.yarnrc`, `.yarnrc.yml`, `.pnpmfile.cjs` or `.envrc` rewrites where an install resolves from and what credential it presents, and it does so from a file a reviewer skims past — and a `.pnpmfile.cjs` executes. The guard (`git ls-files | grep -E '(^|/)(\.npmrc|\.yarnrc(\.yml)?|\.pnpmfile\.cjs|\.envrc)$'`, failing the job when the grep prints anything) runs before the install in `ci.yml`'s `build-and-test` and in `release.yml`'s `build` and `review`. It carries no `if:` and no `continue-on-error:`, because a guard that can be skipped is not a guard, and the workflow-parsing test asserts both the ordering and their absence. There are zero tracked paths of this class today; the guard was proven able to fail on a scratch repository carrying one `.npmrc`, where it exits 1 and names the path.
- **`ci.yml` now runs on changes to `scripts/**` and `.github/workflows/**`.** The suite asserts things *about* those files — the release job order, the publish job's permissions, the guard before every `npm ci`, the behaviour of `release-artifact-review.mjs` — and the path filter excluded both, so a pull request editing only `release.yml` ran no tests at all. Same defect, and same reasoning, as the `package-lock.json` omission recorded below.
